...
- Ensure the `PROBO_API_TOKEN` environment variable is set correctly
...
If you manage multiple Probo organizations:
...
```json
{
  "mcpServers": {
    "probo-org1": {
      "command": "npx",
      "args": ["-y", "@modelcontextprotocol/server-fetch", "https://org1.probo.com/api/mcp"],
      "env": {
        "PROBO_API_TOKEN": "token_for_org1"
      }
    },
    "probo-org2": {
      "command": "npx",
      "args": ["-y", "@modelcontextprotocol/server-fetch", "https://org2.probo.com/api/mcp"],
      "env": {
        "PROBO_API_TOKEN": "token_for_org2"
      }
    }
  }
}

  ...
  "mcpServers": {
    ...
    "probo-staging": {
      ...
      "args": [
        "-y",
        "@modelcontextprotocol/server-fetch",
        "https://staging.probo.com/api/mcp"
      ],
      "env": {
    ...
    "probo-prod": {
      ...
      "args": [
        "-y",
        "@modelcontextprotocol/server-fetch",
        "https://probo.company.com/api/mcp"
      ],
      "env": {
...
```json
{
  ...
  "windsurf.ai.contextSources": ["probo"],
  ...
  useEffect(() => {
    async function fetchMetrics() {
      ...
      setMetrics({
        ...
        implementedMeasures: measures.filter((m) => m.state === "IMPLEMENTED")
          .length,
        overdueObligations: obligations.filter(
          (o) => new Date(o.due_date) < new Date(),
        ).length,
      });
    }
